...
def test_produtoFisico_heranca():
    try:
        ...
        raise AssertionError('Erro ao criar ProdutoFisico')
    assert isinstance(prod, loja.Produto) and isinstance(prod,
                                                         loja.ProdutoFisico), 'A classe ProdutoFisico deve herdar da classe Produto'
...
def test_produtoFisico_caracteristicas_herdadas():
    try:
        ...
        raise AssertionError('Erro ao criar ProdutoFisico')
    assert ('_Produto__nome' in dict_attrs_obj) and ('_Produto__preco' in dict_attrs_obj) and (
                '_ProdutoFisico__nome' not in dict_attrs_obj) and (
...
def test_produtoEletronico_heranca():
    ...
    assert isinstance(prod, loja.ProdutoFisico) and isinstance(prod,
                                                               loja.ProdutoEletronico), 'A classe ProdutoEletronico deve herdar da classe ProdutoFisico'
...
def test_produtoEletronico_caracteristicas_herdadas():
    try:
        ...
        raise AssertionError('Erro ao criar ProdutoEletronico')
    assert ('_Produto__nome' in dict_attrs_obj) and ('_Produto__preco' in dict_attrs_obj) and (
                '_ProdutoFisico__peso' in dict_attrs_obj) and ('_ProdutoEletronico__nome' not in dict_attrs_obj) and (
                       '_ProdutoEletronico__preco' not in dict_attrs_obj) and (
                       '_ProdutoEletronico__peso' not in dict_attrs_obj), 'A classe ProdutoEletronico não deve possuir os atributos privados nome, preco e peso'
...
@pytest.mark.parametrize('nome,preco,peso,tensao,tempo_garantia',
                         [('Cafeteira', 300, 1500, 127, 6), ('Geladeira', 3500, 25000, 220, 12),
                          ('Televisao', 4000, 8500, 0, 24)])
def test_cria_produtoEletronico_inicializado_corretamente(nome, preco, peso, tensao, tempo_garantia):
    ...
    assert hasattr(prod, "_ProdutoFisico__peso"), "A classe ProdutoFisico não possui o atributo privado peso"
    assert hasattr(prod,
                   "_ProdutoEletronico__tensao"), "A classe ProdutoEletronico não possui o atributo privado tensao"
    assert hasattr(prod,
                   "_ProdutoEletronico__tempo_garantia"), "A classe ProdutoEletronico não possui o atributo privado tempo_garantia"
...
def test_ebook_heranca():
    ...
    assert isinstance(prod, loja.Produto) and isinstance(prod,
                                                         loja.Ebook), 'A classe Ebook deve herdar da classe Produto'
...
def test_ebook_caracteristicas_herdadas():
    try:
        ...
        raise AssertionError('Erro ao criar Ebook')
    assert ('_Produto__nome' in dict_attrs_obj) and ('_Produto__preco' in dict_attrs_obj) and (
                '_Ebook__nome' not in dict_attrs_obj) and (
...
@pytest.mark.parametrize('nome,preco,autor,numero_paginas',
                         [('Aprendendo Python', 30, 'Joao Santos', 150), ('Learning Java', 250, 'John da Silva', 810)])
